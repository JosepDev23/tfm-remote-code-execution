...
} from './c-container.errors'
import { JwtStrategy } from 'src/auth/strategies/jwt.strategy'
import { Cron, CronExpression } from '@nestjs/schedule'

@Injectable()
export class CContainerService implements OnModuleDestroy {
  ...
  async createUserContainer(user: Express.User): Promise<string> {
    ...
    const containerName: string = `c-${uuidv4()}`
  ...
  async executeCode(
    ...
    const exec = await container.exec({
      AttachStdout: true,
      AttachStderr: true,
      // Compile and run: gcc tmp.c -o tmp && ./tmp
      Cmd: ['sh', '-c', 'gcc tmp.c -o tmp 2>&1 && ./tmp'],
  ...
  private async cleanupInactiveContainers() {
    ...
    for (const [userId, last] of this.lastActivity.entries()) {
      if (now - last > this.INACTIVITY_LIMIT) {
        ...
        console.log(
          `🧹 C container for user ${userId} removed due to inactivity`,
        )
      }
    }
  }
